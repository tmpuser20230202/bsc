...
outdir = './output/kdd2023/real/flightlist/week'
...
with open('data/real/FLIGHTLIST_20190901_20200531/week/count_od_by_week.pkl', 'rb') as f:
    count_od = pickle.load(f)

weeks_array = sorted(count_od['day'].unique())

airports_array = np.unique(np.hstack((count_od['origin'].unique(), count_od['destination'].unique())))

g_list = [] 
vp_list = []
for week in weeks_array:
    edges = count_od.loc[count_od['day']==week, ['origin', 'destination']].values
...
states_list = []
for week, g in tqdm.tqdm(zip(weeks_array, g_list)):
    print(week)
    states_list_month = []
    for k in k_list:
        states = gt.minimize_blockmodel_dl(g, deg_corr=False, B_min=k, B_max=k)
        states_list_month.append(states)
        
    states_list.append(states_list_month)
